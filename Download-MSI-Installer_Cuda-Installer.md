- > VisualGuide Installer Version: 1.0.31 ( outdated )
  - > (`outdated`): 1.0.40 
  - > (`outdated`): 1.0.41 (dependency updates)
  - > (`outdated`): 1.0.43 (major updates)
  - > (`outdated`): `1.0.50`
  - > (`outdated`): `1.0.54` (Dependency Updates & Security Updates)

  - > `New Version`: `1.0.60` (Dependency Updates & Security Updates)
---

- > Nvidia Cuda Cudnn Installer Version: 0.0.3 (`outdated`)
- > Nvidia Cuda Cudnn Installer Version: `0.0.5` (`outdated`)
- > Nvidia Cuda Cudnn Installer Version: `0.0.10` (security updates)


- > *dont install in program files*
- > *install in appdata*



### [Download VisualGuide](https://www.fnbubbles420.org/Visual-Guide/visual-guide-info)
